...
Component Design #1 (classic Graph):

- Justification:

   1. Time constraints: This design is the simplest to implement quickly (adjacency list with a Map<V, Set<V>>), while still demonstrating meaningful behavior.

   2. Client value: It’s the most intuitive API for a client (add vertices/edges, query existence, run reachability), which makes the PoC compelling in main.

   3. Risk management: It avoids “cursor state” complexity (Design #3) and avoids kernel-level disassemble/assemble constraints (Design #2), both of which add design ambiguity during a quick MVP.

   4. Adaptability to OSU discipline: The methods naturally split into kernel-like operations (add/remove/contains) and secondary-like operations (order/size/reachability), so it will be straightforward to refactor into Standard + kernel + secondary later.
